...
(defun gen-graph (dir)
  (let* ((files (directory (format nil "~A/*.esl" dir))))
    (mapcar #'get-depends files)))

(defun main (esl-dir)
  (let* ((graph (gen-graph esl-dir)))
    (multiple-value-bind (sorted sorted?)
        (topological-sort graph)
      (if sorted?
          (format t "~{~a~^ ~}~%" (remove-duplicates sorted :test #'equal))
          (signal 'sort-error :graph graph)))))
